...
class KoszykRepository @Inject()(dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) {
  ...
  //private val prod = TableQuery[UserTable]
   ...
   class KoszykTable(tag: Tag) extends Table[Koszyk](tag, "koszyk") {
    ...
    /**
      ...
      */
    def * = (id, user, product,productName, liczba) <> ((Koszyk.apply _).tupled, Koszyk.unapply)
  ...
  def create(user1: String,product1: Int,productName:String,liczba:Int): Future[Boolean] = {
    println(user1);
        ...
        db.run(koaszyk.filter(_.product === product1).exists.result).flatMap { exists1 =>
          if(exists1) {
            ...
            db.run {
              ...
              (koaszyk.map(p => (p.user, p.product,p.productName, p.liczba))

                returning koaszyk.map(_.id)

                into {case ((user1,product1,productName,liczba),id) => Koszyk(id,user1,product1,productName,liczba)}

                ) += (user1, product1,productName, liczba)
            }.map(res => true)
          }
        }
      }
