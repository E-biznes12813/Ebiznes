...
class KoszykController @Inject()(koszykRepo: KoszykRepository,
  ...
  def addKoszyk = Action.async { implicit request =>
    ...
    koszykForm.bindFromRequest.fold(
      ...
      koszyk => {
        koszykRepo.create(koszyk.user, koszyk.product,koszyk.productName, koszyk.liczba).map { _ =>
...
/**
  ...
  */
case class CreateKoszykForm(user: String, product: Int, productName: String, liczba: Int)
